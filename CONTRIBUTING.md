...
# Install dependencies (skip postinstall — Electron binary isn't needed for lint)
...
**Requirements:** Node.js ≥18 (CI uses Node 20 LTS), npm. The Electron binary downloads on `npm start` but is **not** needed for lint.
...
2. Test manually on Linux and/or Windows if your change affects the UI or game launch.
3. If you added a new feature, update the relevant docs (`README.md`, `ARCHITECTURE.md`, `FLASH_SETUP.md`). The README is bilingual (EN + collapsible PT-BR section) — update both halves when touching user-facing copy.
