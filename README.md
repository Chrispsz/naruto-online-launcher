...
[![Tests](https://img.shields.io/badge/tests-1240%20passing-10b981?style=flat-square)](#-development)
...
├── preload.js           # contextBridge: window.narutoLauncher + window.__SHINOBI_DEBUG__
...
```bash
npm install --ignore-scripts   # skip the ~180MB Electron binary if you only lint/test
npm test                       # Jest — 1240 tests across 45 suites
npm run lint                   # ESLint (legacy config pinned via ESLINT_USE_FLAT_CONFIG=false)
```

Contributions welcome — see **[CONTRIBUTING.md](CONTRIBUTING.md)**. Golden rule: lint stays clean and all 1240 tests pass.
