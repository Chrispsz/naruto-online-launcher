...
### Changed
- **Polish (Caretaker cycle 8)**: added missing JSDoc to `_winPrioConstants()` in `CpuOptimizer.js`; translated 4 Portuguese JSDoc comments to English across `CpuOptimizer.js` (bitmask, nice mapping, OOM, PID param, affinity comment); translated 3 Portuguese JSDoc comments in `SessionLifecycle.js` (module header, reloadWithPreAuth, _loadGameWithPreAuth); fixed typo "BEFORE from loadURL" → "BEFORE loadURL" in SessionLifecycle.js; fixed truncated comment in `Auditor.js` (`// ensures` → complete sentence); translated 1 Portuguese JSDoc in `api-login.js` (renewIfNeeded threshold param). All protected i18n data untouched.
